...
public class AhenkInstallationUtil {
	...
	public static void installAhenk(final AhenkSetupConfig config, final ProgressBar progressBar, final Table table, Label label) {
		...
		if (config.isUseUsernameAndPass()) {
				...
				for (int i = 0; i < config.getIpList().size(); i++) {
					...
					Display.getCurrent().asyncExec(new Runnable() {
						...
						public void run() {
							...
							try {
								SetupUtils.installPackage(config.getIpList().get(j), config.getUsernameCm(), config.getPasswordCm(), null, null, debFile, false);
			...
			if (config.isInstallByAptGet()) {
				...
				//TODO Caner Feyzullahoğlu: no need to read from file. Can be removed later.
				...
				for (int i = 0; i < config.getIpList().size(); i++) {
					...
					Display.getCurrent().asyncExec(new Runnable() {
						...
						public void run() {
							try {
								SetupUtils.installPackage(config.getIpList().get(j), "root", null, null, config.getPrivateKeyAbsPath(), debFile, false);
