package tr.org.pardus.mys.liderahenksetup.ahenk.statusdialog;
	...
	protected Control createDialogArea(Composite parent) {
		...
		startButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				label.setText(Messages.getString("INSTALLING_AHENK_TO_GIVEN_MACHINES" + " " + Messages.getString("PLEASE_WAIT")));
...
//				true);
		createButton(parent, IDialogConstants.CANCEL_ID,
				Messages.getString("CLOSE"), false);
	}
	...
	private void populateTable() {
		for (int i = 0; i < config.getIpList().size(); i++) {
			...
			item.setText(new String[] {config.getIpList().get(i), Messages.getString("WAITING_TO_START_INSTALLATION")});
		}
	}
}
