...
public class LiderComponentSelectionPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		question = GUIHelper.createLabel(mainContainer, Messages.getString("WHICH_COMPONENTS_WILL_BE_INSTALLED"));
		...
		checkAll = GUIHelper.createButton(mainContainer, SWT.CHECK, Messages.getString("ALL(RECOMMENDED)"));

		checkMariaDb = GUIHelper.createButton(mainContainer, SWT.CHECK,
				Messages.getString("MARIA_DB_VERSION") + " 1.0");

		checkLdap = GUIHelper.createButton(mainContainer, SWT.CHECK, Messages.getString("OPENLDAP_VERSION") + " 1.0");

		checkEjabberd = GUIHelper.createButton(mainContainer, SWT.CHECK,
				Messages.getString("EJABBERD_VERSION") + " 1.0");

		checkLider = GUIHelper.createButton(mainContainer, SWT.CHECK, Messages.getString("LIDER_VERSION") + " 1.0");
	...
	private void setSelectionOfCheckAll() {
		...
		if (checkMariaDb.getSelection() && checkLdap.getSelection() && checkEjabberd.getSelection()
				&& checkLider.getSelection()) {
	...
	public IWizardPage getNextPage() {
		...
		disableNotSelectedComponents(
				(LiderLocationOfComponentsPage) getWizard().getPage(LiderLocationOfComponentsPage.class.getName()));
	...
	public void disableNotSelectedComponents(LiderLocationOfComponentsPage page) {
		...
		// Else set to enabled.
			...
			setEnableStyle(page.getMariaDb(), page.getMariaDbIp(), page.getInstallDistributed());
		}

		if (!(config.getComponentsToBeInstalled().get("LDAP"))) {
			setDisableStyle(page.getLdap(), page.getLdapIp());
		} else {
			setEnableStyle(page.getLdap(), page.getLdapIp(), page.getInstallDistributed());
		}

		if (!(config.getComponentsToBeInstalled().get("Ejabberd"))) {
			setDisableStyle(page.getEjabberd(), page.getEjabberdIp());
		} else {
			setEnableStyle(page.getEjabberd(), page.getEjabberdIp(), page.getInstallDistributed());
		}

		if (!(config.getComponentsToBeInstalled().get("Lider"))) {
			setDisableStyle(page.getLider(), page.getLiderIp());
		} else {
			setEnableStyle(page.getLider(), page.getLiderIp(), page.getInstallDistributed());
		}
	...
	private void setDisableStyle(Label label, Text text) {
		...
		label.setForeground(new Color(getShell().getDisplay(), new RGB(158, 158, 158)));

		label.setFont(new Font(getShell().getDisplay(), label.getFont().toString(), 9, SWT.ITALIC));

		text.setText(Messages.getString("NOT_SELECTED_FOR_INSTALLATION"));

		text.setEnabled(false);

		text.setFont(new Font(getShell().getDisplay(), label.getFont().toString(), 9, SWT.ITALIC));
	}

	// Set a component's style to enabled
	private void setEnableStyle(Label label, Text text, Button button) {

		label.setForeground(new Color(getShell().getDisplay(), new RGB(33, 33, 33)));

		label.setFont(new Font(getShell().getDisplay(), label.getFont().toString(), 9, SWT.NORMAL));
		...
		text.setFont(new Font(getShell().getDisplay(), label.getFont().toString(), 9, SWT.NORMAL));
	}
