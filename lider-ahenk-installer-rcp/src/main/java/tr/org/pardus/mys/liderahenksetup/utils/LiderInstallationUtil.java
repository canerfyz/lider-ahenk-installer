...
public class LiderInstallationUtil {
	...
	private static final String DELETE_EXISTING_QUESTIONS = 
			"su -c 'echo PURGE | debconf-communicate {0}'";

	private static final String DEBIAN_FRONTEND_NONINTERACTIVE = 
				...
				if (SetupUtils.canConnectViaSsh(config.getMariaDbIp(), 
					...
					try {
						...
						if (SetupUtils.packageExists(config.getMariaDbIp(), config.getMariaDbSu(), 
							...
							// Uninstall all mariadb packages to clear all questions from debconf.
							SetupUtils.uninstallPackage(config.getMariaDbIp(), config.getMariaDbSu(), 
									config.getMariaDbSuPass(), null, null, "mariadb-*");
							logger.log(Level.INFO, 
									"Installer successfully uninstalled mariadb packages.");
...
//		sudo debconf-set-selections <<< 'mariadb-server-10.0 mysql-server/root_password password 1'
//		sudo debconf-set-selections <<< 'mariadb-server-10.0 mysql-server/root_password_again password 1'
	...
	private static void deleteExistingQuestions(String ip, String username,
			final String password, Integer port, String privateKey,
			String packageName) throws SSHConnectionException,
			CommandExecutionException {
			// Check if the target is local or remote
			if (NetworkUtils.isLocal(ip)) {
				try {
					logger.log(Level.INFO, 
							"Locally deleting existing questions for package: {0}",
					...
					// TODO eğer lokale sshla (privatekeyle) bağlanacaksam case'ini ele almak lazım.
					if (password != null) {
					...
					// TODO else bloğu silinecek
						...
						logger.log(Level.SEVERE, 
								"Process ends with exit value DENEME DENEME: {0}- err: {1}",
								new Object[] { process.exitValue(), 
								StringUtils.convertStream(process.getInputStream() )});
					}
					
					logger.log(Level.INFO, 
							"Local existing questions for package: {0} have been deleted successfully.", 
							new Object[] { packageName });
				} catch (IOException e) {
					e.printStackTrace();
				...
				try {
					logger.log(Level.INFO, 
							"Remotely deleting existing questions for package: {0}",
							new Object[] { packageName });
					
					// If target is remote connect with SSH
					SSHManager manager = new SSHManager(ip, username, password, 
							port, privateKey);
					...
					// TODO burada da SSH'la bağlandığımda username ve password yok elimde 
					// buna göre düzenlenmesi lazım
//					outputStream = manager.execCommand(DELETE_EXISTING_QUESTIONS, new Object[] { 
...
//					});
					...
					logger.log(Level.INFO, 
							"Remote existing questions for package: {0} have been deleted successfully.", 
							new Object[] { packageName });
				} catch (CommandExecutionException | SSHConnectionException e) {
					e.printStackTrace();
				}
